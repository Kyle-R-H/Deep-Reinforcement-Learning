...
#     "1"  # Used for debugging; CUDA related errors shown immediately.
# )
...
class ReplayMemory:
    ...
    def store(self, state, action, next_state, reward, done):
        """
        Append the transitions to their respective deques
        """
...
class DQN_Agent:
    ...
    def __init__(
        ...
        self.main_network = DQN_Network(num_actions=output_dim, input_dim=input_dim).to(
            device
        )
        self.target_network = (
            DQN_Network(num_actions=output_dim, input_dim=input_dim).to(device).eval()
        )
    ...
    def learn(self, batch_size, done):
        ...
        with torch.no_grad():
            next_target_q_value = self.target_network(next_states).max(
                dim=1, keepdim=True
            )[
                0
        ...
        y_js = rewards + (
            self.discount * next_target_q_value
...
class step_wrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        state, reward, done, truncation, info = self.env.step(
            action
        )  # Same as before as usual

        modified_state = self.observation_wrapper.observation(
            state
        )  # Give the state to another Wrapper, which returns a modified version of state
        modified_reward = self.reward_wrapper.reward(
            modified_state
        )  # Give the modified state to another Wrapper to return the modified reward
